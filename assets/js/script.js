...
window.onload = function () {
  ...
  // Random para Definir Personaje
  const personajeRandom = Math.floor(Math.random() * 12) + 1;
  document.getElementById("random").innerHTML = personajeRandom;
...
function preguntar() {
  ...
  const randomPregunta = parseInt(document.getElementById("random").innerHTML);

  opciones = document.formulario.color; //acceso a los botones radio
  for (i = 0; i < opciones.length; i++) {
    //buscar valor del botón pulsado
  ...
  switch (elegido) {
    //elegir color según boton pulsado
    case "esHombre":
      if (
        randomPregunta === 2 ||
        randomPregunta === 3 ||
